...
| `环境与踩坑记录.md` | 跨机器、跨平台的环境问题与解法 |
| `scripts/` | `bootstrap-workspace.{sh,ps1}` 一键拉齐七个仓库；`run-on-tx.sh` 在服务器上执行 |
| `总节奏表.md` | 13 周 × 6 项目排期与工时预算 —— **尚未创建** |
| `进度总览.md` | 各项目进度汇总看板 —— **尚未创建** |

> ⚠️ 后两个文件目前不存在，但已被本仓库和六个项目仓库的 `AGENTS.md` / `CLAUDE.md` / `docs/progress.md` 共 21 处引用为写入目标。新建它们之前，那些「排期写到 `总节奏表.md`」「进度汇总到 `进度总览.md`」的指引无处落地。
