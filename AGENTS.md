...
仓库内的日常记录（`docs/progress.md`、`AGENTS.md` 等）不在此列，照常直接写。

**1. 写入前先问**：说明**标题、大纲、具体目录**（落在哪个节点下、需要新建哪几层），得到确认再写。新建目录节点同样要先确认。

理由：飞书文档是以用户本人身份创建的对外产物，未经确认就发出去，等于替他做了外发决定。

**2. 定位置：先复用，再新建**。优先复用已有的合适目录，同类文档集中归档，不要平行开新目录；`wiki +node-list` 确认没有合适的，再建节点、然后把文档建进去 —— **先目录后文档，不要先建再搬**。

**3. 写入后核对**：回查确认文档**真的落在目标节点下**（`wiki +node-list --parent-node-token <目标>`），不能只看创建命令返回 `ok: true`；移动是异步任务还要确认 `status_msg`。然后把**最终链接**交给用户，移动过的要给新链接。
